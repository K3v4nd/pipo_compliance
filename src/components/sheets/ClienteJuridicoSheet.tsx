...
import { ClienteJuridicoData, CompanyConfig } from '../../types';
...
export const ClienteJuridicoSheet: React.FC<Props> = ({ data, company }) => {
  return (
    <div 
      id="printable-sheet-cliente-juridico" 
    ...
    >
      ...
      <SheetHeader 
        company={company}
        title="REGISTRO CLIENTES JURIDICOS"
      ...
      <div className="mt-1 border-2 border-black">
        ...
        <div className="grid grid-cols-12 border-b border-black">
          <div className="col-span-5 p-1 border-r border-black">
            <span className="font-bold text-[7.5px] uppercase text-gray-700 block">2. RAZON SOCIAL:</span>
            <span className="font-bold text-[9.5px] uppercase block">{data.razonSocial || '\u00A0'}</span>
          </div>
          ...
          <div className="col-span-1 p-1 border-r border-black text-center">
            ...
            <span className="font-bold text-[9px] block">{data.docTipo || 'RIF'}</span>
          </div>
      ...
      <div className="mt-1 border-2 border-black">
        ...
        <div className="p-1.5 text-[8.5px] text-justify leading-snug border-b border-black">
          Declaro que los productos y servicios ofrecidos por nuestra empresa provienen de capital lícito obtenido, por lo que autorizo de manera expresa a las autoridades competentes a la verificación de la información entregada en este formulario.
        </div>
      ...
      <div className="mt-1 border-2 border-black">
        ...
        <div className="grid grid-cols-12 border-b border-black text-[8.5px] items-center">
          <div className="col-span-9 px-2 py-0.5">Fotocopia de Documento Constitutivo</div>
          <div className="col-span-1.5 text-center border-l border-black font-bold py-0.5">{data.docConstitutivo ? 'X' : ''}</div>
          <div className="col-span-1.5 text-center border-l border-black font-bold py-0.5">{!data.docConstitutivo ? 'X' : ''}</div>
        </div>

        <div className="grid grid-cols-12 border-b border-black text-[8.5px] items-center">
          <div className="col-span-9 px-2 py-0.5">Fotocopia del RIF o pasaportes de Socios y/ Representantes legales</div>
          <div className="col-span-1.5 text-center border-l border-black font-bold py-0.5">{data.docRifSocios ? 'X' : ''}</div>
          <div className="col-span-1.5 text-center border-l border-black font-bold py-0.5">{!data.docRifSocios ? 'X' : ''}</div>
        </div>

        <div className="p-1 min-h-[22px]">
          <span className="font-bold text-[7.5px] uppercase text-gray-700 block">OBSERVACIONES:</span>
          <span className="text-[8.5px] italic text-gray-800">{data.observaciones || 'Ninguna observación.'}</span>
        </div>
      </div>
